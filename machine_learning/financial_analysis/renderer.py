...
def renderIndicators(interpolated_merged_df):
    i = 0.075

    #trunca las filas a una de cada 365
    interpolated_merged_df = interpolated_merged_df[::2]
    fig, ax1 = plt.subplots()
    #indice = interpolated_merged_df.index
    indice = interpolated_merged_df["Date"]
    ...
    #ax1.xaxis.set_ticks(interpolated_merged_df["Date"])
    ax1.set_xticklabels(interpolated_merged_df["Date"], rotation=45, ha="right")
    ax1.set_ylabel('United States Broad money growth', color = color)
    ax1.plot(indice, interpolated_merged_df["United States Broad money growth"], color = color)
    ax1.tick_params(axis='y', labelcolor = color )

    ax2 = ax1.twinx()  # instantiate a second axes that shares the same x-axis
    color = 'tab:blue'
    ax2.set_ylabel('United States Adjusted net national income', color = color)  # we already handled the x-label with ax1
    ax2.plot(indice, interpolated_merged_df["United States Adjusted net national income"], color=color)
    ax2.tick_params(axis='y', labelcolor=color)
